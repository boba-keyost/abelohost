{if $sort}
    ...
    {append var='title' value='Default' index='id'}
    ...
    {block name="sorts" assign="content"}
        <ul class="sort">
            {foreach $sort->getList() as $field}
                ...
                {/block}
                <li class="item{if $field->isCurrent()} current{/if}">
                    {include file="components/link.tpl" content=$link_content class="sort-link order-{$field->getOrder()|lower}" href="?"|cat:$field->queryParam("sort", $query_parameters)}
                </li>
            {/foreach}
        </ul>
    {/block}
